...
Expressions in this project are C#. Global Constants are referenced qualified, as
`GlobalConstants.<Name>`, names are case-sensitive, and every expression lives in a `<CSharpValue>`
or `<CSharpReference>` element rather than an attribute.
